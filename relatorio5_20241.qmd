...
O laboratório desta semana visa a continuidade da aplicação das técnicas de Regressão Linear Simples com a aplicabilidade das técnicas de análise de resíduos e transformação de variáveis para a condição de quebra dos pressupostos do modelo.
...
O conjunto de dados a ser analisado é denominado *trees*, disponível no pacote *datasets*, contém informações de 31 cerejeiras (*Black cherry*) da Floresta Nacional de Allegheny, relativas a três características numéricas contínuas:
...
dados1|>
  ...
  geom_point(shape=1, color = "#234B6E")+
  labs(
    title = "Figura 1: Modelo Ajustado entre Volume e Altura",
    subtitle = "Diagrama de dispersão com equação da reta de regressão ajustada, \nCoeficiente de Correlação de Pearson, Coeficiente de Determinação, Coeficiente de \nDeterminação Ajustado, Reta de Regressão e Intervalo de Confiança",
    y = 'Volume (m³)', x = 'Altura (m)'
  )+
  ggpubr::stat_cor(
    aes(label = paste(..r.label.., ..rr.label.., sep = "~`; `~")),
    cor.coef.name = c("R"),
    label.sep = "; ", geom = "text", color="tomato", method = "pearson",
    label.x = 19.2, label.y = 2, show.legend = F,
    p.accuracy = 0.001, r.accuracy = 0.0001, size = 3.5)+
  ggpubr::stat_regline_equation(
      aes(label = paste(..eq.label.., ..adj.rr.label.., sep = "~`; `~")),
          geom = "text", label.x = 19.2, label.y = 1.75, 
      position = "identity", 
      color="tomato", size = 3.5, show.legend = F
  )+
  geom_smooth(
...
Com base na Figura 1 é possível sugerir uma relação positiva entre as variáveis **Volume** e **Altura**, fato confirmado pelo Coeficiente de Correlação de Pearson (*R = `r scales::number(cor(dados1$volume, dados1$height), accuracy = 0.001, big.mark = ".", decimal.mark = ",")`*) que após o teste de hipótese para avaliar a significância da correlação estimada, demonstrou **possuir correlação não nula**. A reta de regressão ajustada segue a seguinte equação:
...
Através dos Intervalos de Confiança calculados (Tabela 3) é possível afirmar **com 95% de confiança que o verdadeiro valor de $\beta_0$ está entre `r glue::glue('({scales::number(confint(mFit)[1,1], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")}; {scales::number(confint(mFit)[1,2], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")})')` e que o verdadeiro valor de $\beta_1$ está entre `r glue::glue('({scales::number(confint(mFit)[2,1], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")}; {scales::number(confint(mFit)[2,2], accuracy = 0.0001, big.mark = ".", decimal.mark = ",")})')`**.
...
d1 + d2 + d3 +
  plot_layout(ncol = 2) +
  plot_annotation(
    title = "Figura 6: Gráficos dos Resíduos Estudentizados vs. Valores Ajustados \ndas características transformadas.",
